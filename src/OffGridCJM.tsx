...
export default function OffGridCJM() {
  ...
  const [landArea, setLandArea] = useState<number>(5000);
  const [landPrice, setLandPrice] = useState<number>(0);
  const [homeArea, setHomeArea] = useState<number>(0);
  const [selectedFoodSystems, setSelectedFoodSystems] = useState<SelectedFoodSystem[]>([]);
  ...
  return (
    <ModalContainer step={step}>
      ...
      {step === "subregions" && (() => {
        const scoredSubregions: (Subregion & { score: number })[] = subregionsData.map((subregion) => {
          const allTags = [
          ...
          const score = allTags.filter((tag) => selectedTags.includes(tag)).length;
      ...
      {step === "receipt" && (() => {
        ...
        const totalAvgTime = avgTimeItems.reduce((sum, item) => sum + item.value, 0);
        const totalPeakTime = peakTimeItems.reduce((sum, item) => sum + item.value, 0);
        
        const resourceTimeItems = timeItems.filter(item => 
          !item.label.includes("(avg)") && !item.label.includes("(peak)")
        );
        const totalResourceTime = resourceTimeItems.reduce((sum, item) => sum + item.value, 0);
        ...
        return (
          <PageLayout>
                      ...
                      return (
                        <div key={index} className="flex justify-between text-xs">
                          ...
                          <span className="font-medium text-blue-700">
                            {item.value} hrs/week (peak: {peakItem?.value || item.value} hrs)
                          </span>
                        </div>
                      );
                    })}
            ...
            <div className="text-center mt-12">
              <p className="text-xs text-gray-600 mb-6">Your journey is complete!</p>
              <Button
                onClick={() => {
                  setStep("welcome");
                  stableUpdateCart([]);
